...
#define TCP_PORT_STREAM  1004  /* conexion dedicada, empuja bits demodulados solos */
...
/* ---- Squelch / calibracion / puntos de prueba (GMSK_Demod) ----
 * cfg_data se amplio a 128 bits del lado del Block Design. Mapeo
 * asumido (word-aligned, VERIFICAR contra el HDL real de axi_hub):
 *   cfg[0] -> igual que siempre (PINC + reset/run del LO)
 *   cfg[1] -> reservado, sin usar
 *   cfg[2] -> bits[15:0]=umbral_bajo, bits[31:16]=umbral_alto
 *   cfg[3] -> bit[0]=modo (0=normal, 1=calibracion)
 *             bits[3:1]=sel (punto de prueba hacia fifo_0, 0..4)
 */
#define CMD_SET_UMBRALES    0x20   /* payload = (umbral_alto<<16)|umbral_bajo */
#define CMD_SET_CFG3        0x21   /* payload = (sel<<1)|modo */
...
/* ================================================================
 * Hilo de streaming: sondea fifo_0 sin parar (con una espera corta
 * entre chequeos, igual que sdr-transceiver.c de Pavel Demin) y
 * empuja los bits demodulados de cada rafaga apenas detecta que
 * termino de llenarse -- sin resetear el FIFO (a diferencia de
 * CMD_READ_ADC, que si lo resetea a proposito para capturas de ADC
 * crudo). Corre en su propia conexion TCP, en paralelo al hilo
 * principal que sigue atendiendo comandos y configurando el ADRF.
 * ================================================================ */
#define STREAM_MAX_PALABRAS   4096
#define STREAM_POLL_US        500     /* pausa entre sondeos, igual que Pavel */
#define STREAM_GAP_FIN_MS     15.0    /* sin palabras nuevas durante esto,
                                        * recien ahi se da la rafaga por
                                        * terminada -- bastante mas que los
                                        * ~3.3ms que tarda gmsk_bit_pack32 en
                                        * armar cada palabra DURANTE una
                                        * rafaga activa (si no, se corta la
                                        * rafaga en pedazos de una palabra),
                                        * pero bastante menos que el hueco
                                        * real entre rafagas distintas */

static void *stream_handler(void *arg) {
    (void)arg;
    int sock_srv, sock_cli;
    struct sockaddr_in addr;
    int yes = 1;
    uint32_t buf[STREAM_MAX_PALABRAS];

    sock_srv = socket(AF_INET, SOCK_STREAM, 0);
    if (sock_srv < 0) { perror("stream: socket"); return NULL; }
    setsockopt(sock_srv, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes));

    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = htonl(INADDR_ANY);
    addr.sin_port = htons(TCP_PORT_STREAM);
    if (bind(sock_srv, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        perror("stream: bind"); close(sock_srv); return NULL;
    }
    listen(sock_srv, 1);
    printf("Hilo de streaming escuchando en puerto %d\n", TCP_PORT_STREAM);
    ...
    while (!g_stop) {
        ...
        setsockopt(sock_cli, IPPROTO_TCP, TCP_NODELAY, &yes, sizeof(yes));
        printf("Streaming: cliente conectado.\n");

        while (!g_stop) {
            uint32_t disponibles, n_palabras, i;
            struct timespec t_ultima, t_ahora;
            double gap_ms;

            /* esperar a que empiece a llegar algo, sin resetear nada */
            disponibles = sts[0] & 0xFFFFu;
            while (disponibles == 0 && !g_stop) {
                usleep(STREAM_POLL_US);
                disponibles = sts[0] & 0xFFFFu;
            }
            if (g_stop) break;

            /* vaciar seguido (el FIFO real es chico, 16 palabras --
             * hay que drenarlo a medida que se llena, no esperar a
             * que "se estabilice" con el mismo conteo, porque durante
             * una rafaga activa el conteo SIEMPRE esta subiendo de a
             * poco) hasta que pase un hueco largo sin nada nuevo */
            n_palabras = 0;
            clock_gettime(CLOCK_MONOTONIC, &t_ultima);
            while (!g_stop) {
                disponibles = sts[0] & 0xFFFFu;
                if (disponibles > 0) {
                    for (i = 0; i < disponibles && n_palabras < STREAM_MAX_PALABRAS; i++)
                        buf[n_palabras++] = *data_reg;
                    clock_gettime(CLOCK_MONOTONIC, &t_ultima);
                } else {
                    clock_gettime(CLOCK_MONOTONIC, &t_ahora);
                    gap_ms = (t_ahora.tv_sec - t_ultima.tv_sec) * 1e3 +
                             (t_ahora.tv_nsec - t_ultima.tv_nsec) / 1e6;
                    if (gap_ms > STREAM_GAP_FIN_MS) break;
                }
                usleep(STREAM_POLL_US);
            }
            if (g_stop) break;

            /* encabezado simple: cantidad de palabras, despues los
             * datos crudos -- el cliente no sabe de antemano cuantas
             * van a llegar en cada rafaga */
            if (send_all(sock_cli, &n_palabras, sizeof(n_palabras)) < 0) break;
            if (send_all(sock_cli, buf, (size_t)n_palabras * sizeof(uint32_t)) < 0) break;
        }

        close(sock_cli);
        printf("Streaming: cliente desconectado.\n");
    }
...
int main(void) {
    ...
    /* hilo de streaming: corre en paralelo, empuja bits demodulados
     * solo, sin depender de que este hilo principal este libre
     * atendiendo comandos en ese momento */
    {
        pthread_t stream_thread;
        if (pthread_create(&stream_thread, NULL, stream_handler, NULL) != 0) {
            perror("pthread_create (stream_handler)");
            return 1;
        }
    ...
    while (!g_stop) {
        ...
        while (!g_stop) {
            ...
            /* -------- comandos nuevos: squelch / calibracion -------- */
            else if (cmd == CMD_SET_UMBRALES) {
                cfg[2] = payload;
                printf("Squelch: umbral_alto=%u umbral_bajo=%u\n",
                       payload >> 16, payload & 0xFFFFu);
                ack = 0x01;
                send_all(sock_client, &ack, 1);
            }
            else if (cmd == CMD_SET_CFG3) {
                cfg[3] = payload & 0xFu;   /* sel[3:1] + modo[0] */
                printf("cfg3: sel(punto_prueba)=%u modo=%s\n",
                       (payload >> 1) & 0x7u, (payload & 0x1u) ? "calibracion" : "normal");
                ack = 0x01;
                send_all(sock_client, &ack, 1);
            }
