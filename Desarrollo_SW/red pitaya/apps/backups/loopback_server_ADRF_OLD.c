...
/* ---- Squelch / calibracion (GMSK_Demod) ----
 * cfg_data se amplio a 128 bits del lado del Block Design para llevar,
 * ademas del PINC+reset/run de siempre (cfg[0], sin tocar), los umbrales
 * del squelch y el modo de calibracion. Mapeo asumido (word-aligned,
 * VERIFICAR contra el HDL real de axi_hub si algo no cuaja -- por
 * ejemplo escribiendo un valor conocido y confirmando con un ILA que
 * gmsk_squelch_0/umbral_alto y umbral_bajo reciben lo esperado):
...
#define CMD_SET_MODO        0x21   /* payload = 0 (normal) o 1 (calibracion) */
...
int main(void) {
    ...
    while (!g_stop) {
        ...
        while (!g_stop) {
            ...
            else if (cmd == CMD_SET_MODO) {
                cfg[3] = (payload != 0) ? 0x1u : 0x0u;
                printf("Squelch: modo -> %s\n", payload ? "calibracion" : "normal");
                ack = 0x01;
                send_all(sock_client, &ack, 1);
            }
